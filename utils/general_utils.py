...
def _collate_MCAT(batch):
    ...
    label = torch.LongTensor([item[8].long() for item in batch])
    event_time = torch.FloatTensor([item[9] for item in batch])
    c = torch.FloatTensor([item[10] for item in batch])

    clinical_data_list = []
    for item in batch:
        clinical_data_list.append(item[11])

    # mask = torch.stack([item[11] for item in batch], dim=0)

    return [img, graph, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, clinical_data_list]
